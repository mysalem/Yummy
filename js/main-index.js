...
$(".open-close-icon").click(() => {
    if ($(".side-nav-menu").css("left") == "0px") { closeNav() }
    else { openNav() }
});
...
function displayMeals(arr) {
    ...
    rowData.innerHTML = displayStore
}
...
// === Search ===
function showSearch() {
    mainDiv.classList.remove("min-vh-100")
    rowData.innerHTML = ``;
    searchContainer.innerHTML = `
    <div class="row py-4 ">
        <div class="col-md-6 ">
        ...
        <div class="col-md-6">
            <input onkeyup="searchByFirstLetter(this.value)" maxlength="1" class="form-control bg-transparent text-white" type="text" placeholder="Search Meal By First Letter">
        </div>
    </div>`
}

async function searchByName(searchChar) {
    closeNav()
    mainDiv.classList.add("min-vh-100")
    rowData.innerHTML = ""
    $(".inner-loading-screen").fadeIn(300)

    let response = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${searchChar}`)
...
async function searchByFirstLetter(searchChar) {
    closeNav()
    mainDiv.classList.add("min-vh-100")
    rowData.innerHTML = ""
    $(".inner-loading-screen").fadeIn(300)

    searchChar == "" ? searchChar = "a" : "";
    let response = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?f=${searchChar}`)
    response = await response.json()
...
function inputsValidation() {
    ...
    if (passwordInputTouched) {
        ...
        else { document.getElementById("passwordAlert").classList.replace("d-none", "d-block") }
    }
    ...
    if (repasswordInputTouched) {
        ...
        else { document.getElementById("repasswordAlert").classList.replace("d-none", "d-block") }
    }
